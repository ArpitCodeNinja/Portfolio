...
const experiences = [
  {
    id: 1,
    title: "Prodigy Infotech Pvt. Ltd.",
    subtitle: "Data Science Intern",
    period: "August 2025 - September 2025",
    img: "/Experience/Arpit.jpg",
    fit: "cover",
    position: "center",
    scale: 1.1
  }
...
const ExpCarousel = () => {
  ...
  useGSAP(
    () => {
      ...
      const stickyHeight = experiences.length > 1 
        ? window.innerHeight * (experiences.length + 1)
        : window.innerHeight * 0.5; // Much shorter for a single card
        ...
        onUpdate: (self) => {
          ...
          slides.forEach((slide, index) => {
            ...
            if (index === currentSlide || index === currentSlide + 1) {
              ...
              const parallaxAmount = experiences[index].scale === 1 ? 0 : relativeProgress * slideWidth * 0.25;
              const scaleValue = experiences[index].scale || 1.35;

              gsap.set(image, { x: parallaxAmount, scale: scaleValue });
            } else {
              gsap.set(image, { x: 0, scale: experiences[index].scale || 1.35 });
            }
          });
        },
      });
          ...
          {experiences.map((exp, index) => (
            ...
            >
              <div className="ec-img">
                <img
                  src={exp.img}
                  alt={`${exp.title} office environment`}
                  style={{
                    objectFit: exp.fit || "cover",
                    objectPosition: exp.position || "top center"
                  }}
                />
              </div>
              {exp.title && (
